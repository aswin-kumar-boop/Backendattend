...
exports.addClassWithTimetable = async (req, res) => {
  try {
    const {
      ...
      departmentName,
      year,
    } = req.body.classDetails;
    const { semester, sessions, startDate, endDate } =
      req.body.timetableDetails;
    ...
    await timetable.populate(
      "classId",
      "className classCode Class_instructor room"
    );
...
exports.addSession = async (req, res) => {
  try {
    const { semester, year, session, classDetails, startDate, endDate } =
      req.body;
    ...
    const newSessionDuration =
      (new Date(session.endTime) - new Date(session.startTime)) / 3600000; // Convert ms to hours
    ...
    if (timetable) {
      const totalDurationOnDay = timetable.sessions
      ...
      if (
        totalDurationOnDay + newSessionDuration >
        globalSettings.timetable.TotalDuration
      ) {
...
exports.updateSession = async (req, res) => {
  try {
    ...
    if (!timetable) {
      return res
        .status(404)
        .json({ status: "error", message: "Timetable not found" });
    }
    ...
    timetable.sessions.forEach((session) => {
      if (session.day === sessionUpdate.day) {
        if (session._id.toString() === sessionId) {
          ...
          totalDuration +=
            (new Date(sessionUpdate.endTime) -
              new Date(sessionUpdate.startTime)) /
            (1000 * 60 * 60);
        } else {
...
exports.getWeeklyTimetable = async (req, res) => {
  try {
    ...
    const classes = await Class.find(
      ...
      "_id"
    ); // Only fetch class IDs

    if (!classes.length) {
      return res.status(404).json({
        status: "error",
        message:
          "No classes found for the specified department and academic year.",
      });
    }
    ...
    if (!timetables.length) {
      return res.status(404).json({
        status: "error",
        message: "Timetable not found for the specified classes.",
      });
    }
